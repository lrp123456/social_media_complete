...
> 日期：2026-06-19
> 范围：数据源调研（A）、选择器配置系统重构（B）、私密视频过滤（C）、抽屉匹配重构（D）
...
- 私密过滤字段：无 API 字段 → 用 `play_count`（观看数）=0 判断
...
- 字段映射（共享 `parseVideoItem`）：
  ...
  - `comment_count` ← `comment_count` / `metrics.comment_count`
- 分页：`body.has_more` (bool), `body.cursor` (string)
- 私密过滤字段：`photoStatus`（仅 `work_list` 源有此字段，`photo_analysis` 源无此字段）
...
- 私密过滤字段：`permission_code !== 0` 为私密（已实现）
- 去重：单一数据源，无需跨源去重
...
- 私密过滤字段：`visibleType !== undefined && visibleType !== 1` 为非公开（必须先检查 `undefined`，旧数据可能无此字段）
- 换页按钮选择器：
...
|------|---------|----------|-------------------------------|---------|
| 抖音 | play_count | `statistics.play_count` / `stat.play_count` | `=== 0` 为私密（`undefined` 视为公开，不过滤） | ❌ 缺失 |
| 快手 | photoStatus | `photoStatus` | `!== undefined && !== 0` 为私密（`photo_analysis` 源无此字段，必须跳过） | ❌ 缺失 |
| 小红书 | permission_code | `permission_code` | `!== 0` 为私密（已实现） | ✅ 已实现 |
| 视频号 | visibleType | `visibleType` | `!== undefined && !== 1` 为非公开（旧数据可能无此字段） | ❌ 缺失 |

**关键约束**：所有过滤条件必须先检查 `!== undefined`，否则缺失字段会被误判为私密，导致整源视频被误杀。
...
2. 切片过滤：
   - `photoStatus !== undefined && photoStatus !== 0` → 排除（私密/异常）
   - `photoStatus === undefined` → **保留**（字段缺失视为公开，`photo_analysis` 源无此字段）

3. 已入库视频动态剔除：同抖音逻辑

4. `photo_analysis` 源天然无私密，但因 raw items 无 `photoStatus` 字段，过滤逻辑会安全跳过（`undefined` 不过滤）
