...
class LoginTabRegistry {
    ...
    async unregister(windowId, flowId) {
        ...
        if (record) {
            ...
            try {
                ...
                if (!loginDomain || currentUrl === 'about:blank')
                    return;
        ...
        try {
            ...
            if (timeoutOverlay) {
                ...
                if (refreshBtn) {
                    ...
                    try {
                        await page.waitForSelector('.qrcode-status-timeout', { state: 'hidden', timeout: 5000 });
                    }
                    catch { /* 可能已消失 */ }
    ...
    async openLoginTab(windowId, userId, flowId, browser, config) {
        ...
        try {
            ...
            await page.evaluate(({ data, markKey }) => {
            ...
            if (page) {
                try {
                    await page.close();
                    console.info('[LoginTabRegistry] openLoginTab: closed orphan page after failure');
                }
                catch { /* ignore */ }
            }
